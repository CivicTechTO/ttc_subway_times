...
Request = namedtuple('Request', ['requestid', 'data_', 'stationid', 'lineid', 'all_stations',
                                 'create_date', 'pollid', 'request_date'])
Response = namedtuple('Response', ['requestid', 'id', 'station_char', 'subwayline',
                                   'system_message_type', 'timint', 'traindirection',
                                   'trainid', 'train_message', 'train_dest'])
...
def jsons_to_csv(dir, output_dir, chunksize=2000):
    ...
    for i, g in enumerate(grouper(files, chunksize, fillvalue=None)):
        ...
        pd.DataFrame.from_records(polls, columns=polls[0]._fields)\
            .to_csv(os.path.join(output_dir, 'polls.csv'), index=False, mode=mode, header=header)

        pd.DataFrame.from_records(requests, columns=requests[0]._fields)\
            .to_csv(os.path.join(output_dir, 'requests.csv'), index=False, mode=mode, header=header)

        pd.DataFrame.from_records(responses, columns=responses[0]._fields)\
            .to_csv(os.path.join(output_dir, 'responses.csv'), index=False, mode=mode, header=header)
    ...
    with open(json_path) as f:
        ...
        for poll in json_file:
            ...
            for request in poll['requests']:
                requestid+=1
                requests.append(Request(requestid=requestid, data_=request['data_'],
                                        stationid=request['stationid'], lineid=request['lineid'],
                                        all_stations=request['all_stations'],
                                        create_date=request['create_date'], pollid=pollid,
                                        request_date=request['request_date']))

                for response in request['responses']:
                    responses.append(Response(requestid=requestid, id=response['id'],
    ...
    for result in paginator.paginate(Bucket=bucket):
        for key in result["Contents"]:
            if(os.path.basename(key['Key'])[-7:] == '.tar.gz'):
                file_date = datetime.datetime.strptime(os.path.basename(key['Key'][:-7]), '%Y-%m-%d')
