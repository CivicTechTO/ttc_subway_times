The purpose of this document is to translate the fields Line ID, Station ID, and station_char from codes / IDs to values that will make sense to a TTC transit user.
...
In the data, some station_char values are associated with 2 station IDs and line IDs. This is because when we request data from the API for a station with multiple subway lines, it returns trains details for all subway lines.  
